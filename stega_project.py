...
def stringtobin(mes):
    ...
    binmes=np.unpackbits(binmes)    #Finally we unpack the integer ASCII values
...
def bintostring(binmes):
    ...
    integs=np.packbits(binmes)   #bin to integer which must be the ASCII values
...
Secret messages used will be an string of characters and an image.
We have to take into account that we can modify our square length where the
dct will be applied in order to fit our cover image shape.
However, may be far more efficient to use a larger square length instead of 2, 
8,in terms of embedding space and time required to do every dct.
Although we can't use some  pixels at the borders image, it still increases
its capacity.'''

def encoding(im, binarymes, sqlen):
    '''In this function takes place the encoding process via dct. Inputs:
    cover image (2D-array) that must fit with the sqlen (integer) decided, and
    binarymes (1D- binary array) which is the one containg the secret message.
    Output: stego image (2D-array) that appears to be exactly how the cover
    image looks like, but it actually has our secret image encoded.'''
    ...
    #Num of bits to be embedded from the sec mes
    binarymessagelength=len(binarymes)
    
    #Max bits that can be embedded in our cover im, due to we can't embed bits
    #in the square diagonal, as when we recover the image this coeffitients
    #are still unaltered so then the idct obtain a very good approximation
    #of the original one
    embedbits=np.size(im)*(1-1/sqlen)
    
    #Max num of bytes that can be embedded in our cover im
    embedbytes=embedbits/8  #A byte can either be a pixel or a character
    
    #Max side length that could have an square image
    maxsqrimside=np.uint8(np.sqrt(embedbytes))
    #We have to note that this is just an orientative parameter,
    #we don't have to introduce a square image, it can perfectly be a rectangle
    #and we introduce every bit. Afterwards, when we decode all bits that
    #characterize our secret image we reshape to its original form, but in this
    #case the one who is receiving the image should know its shape,
    #or what's more, the shape could be included in the image, but the receiver
    #must know where to look for this info.
    
    #Print these values
    print('Number of bits to be embedded from our secret message:',\
          binarymessagelength)
    print('Max num of bits that can be encoded in this cover image \
          and square length:',embedbits)
    print('Max num of bytes that can be encoded in this cover image \
          and square length:',embedbytes)
    ...
    #Ensure that the bits to embed fit the capacity of the image
    if (embedbits<binarymessagelength):
    ...
    #Temporary stego image, where we'll have the dct coefficients with the
    #secret message embedded, but it will be idct to obtain the final stego im
    ...
    for ii in np.arange(Nrow//sqlen):
        for jj in np.arange(Ncol//sqlen):
            ...
            square=im[ii*sqlen:(ii+1)*sqlen,jj*sqlen:(jj+1)*sqlen]  #We take every sqlenxsqlen square of the image
            #Apply the normalized DCT to them
            square = dct(dct(square, axis=0,norm='ortho'),axis=1,norm='ortho')   #DCT
            ...
            if (numsecbit==(len(binarymes))):
                dctsteg[ii*sqlen:(ii+1)*sqlen,jj*sqlen:(jj+1)*sqlen]=square[:,:]
                continue
            ...
            for kk in np.arange(sqlen):
                for ll in np.arange(sqlen):
                    ...
                    if (ll!=kk):
                        ...
                        temp2[hashfun]=str(binarymes[numsecbit])
                        
                        #we recover a string from the list created as temp2
                        fbit=''.join(item for item in temp2)
                        
                        #and compute the final value
                        value=int(fbit,2)
                        
                        #Put it back again in the square with its corresponding
                        #sign and the secret bit embeded
    ...
    #the actual stego im
    steg=np.zeros((Nrow,Ncol))
    for ii in np.arange(Nrow//sqlen):
        for jj in np.arange(Ncol//sqlen):
            #Take the squares from the cover image
            square=dctsteg[ii*sqlen:(ii+1)*sqlen,jj*sqlen:(jj+1)*sqlen]  #We take every sqlenxsqlen square of the image
            
            #Apply the normalized IDCT to them
            square = idct(idct(square, axis=0,norm='ortho'),axis=1,norm='ortho')#IDCT
            
            #Constuct the stego im by filling the squares
            steg[ii*sqlen:(ii+1)*sqlen,jj*sqlen:(jj+1)*sqlen]=square[:,:]
...
def decoding(im, binarymeslen, sqlen):
    ...
    for ii in np.arange(Nrow//sqlen):
        for jj in np.arange(Ncol//sqlen):
            #Take the squares from the cover image
            square=decod[ii*sqlen:(ii+1)*sqlen,jj*sqlen:(jj+1)*sqlen]  #We take every sqlenxsqlen square of the image
    ...
    dec_ssim = ssim(db_im, db_recons, data_range=db_recons.max() - db_recons.min())
...
#Cover image
im = 0.299 * im[:, :, 0] + 0.587 * im[:, :, 1] + 0.114 * im[:, :, 2] #Luminance
...
sec_ssim = ssim(db_secim, db_revealsec, data_range=db_revealsec.max() - db_revealsec.min())
...
cover_ssim = ssim(db_normim, steg, data_range=steg.max() - steg.min(), multichannel= True, channel_axis=2)
print('The SSIM comparing Cover Im and Stego Im is:', cover_ssim)

db_steg = np.double(steg)
db_recons = np.double(recons)
steg_ssim = ssim(db_steg, db_recons, data_range=db_recons.max() - db_recons.min(), multichannel= True, channel_axis=2)
